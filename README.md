...
- **SelfUp Integration**: Application update monitoring with support for multiple providers (GitHub, Docker Hub, custom registries)
  - Track application versions across your infrastructure
  - Automated update checking with configurable intervals
  - Visual dashboard showing update status and statistics
  - Integration with Pulse's alert system for update notifications
...
### Application Update Monitoring (SelfUp)

Pulse includes integrated application update monitoring capabilities inspired by SelfUp. Track application versions across your infrastructure and get notified when updates are available.
...
- **Visual Dashboard**: Dedicated SelfUp tab showing all monitored applications and their update status
- **Alert Integration**: Receive notifications through Pulse's existing alert system when updates are available
- **Version Tracking**: Historical view of application versions and update patterns

**Getting Started:**
1. Navigate to the **SelfUp** tab in Pulse
2. Click "Add App" to configure a new application for monitoring
3. Select the provider (GitHub, Docker Hub, etc.) and configure the application details
4. Set your preferred update checking interval
5. Enable alerts to receive notifications when updates are available

The SelfUp integration maintains Pulse's security and privacy standards - all update checking is performed locally with no external telemetry.
...
- [SelfUp Integration](docs/SELFUP.md) - Application update monitoring setup and configuration
